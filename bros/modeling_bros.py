    ...
    def forward(
        ...
        if labels is not None:
            ...
            if box_first_token_mask is not None:
                ...
                loss = loss_fct(logits.view(-1, self.num_labels)[box_first_token_mask], labels.view(-1)[box_first_token_mask])
            else:
                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
    ...
    def forward(
        ...
        if itc_labels is not None and stc_labels is not None:
            ...
            if itc_mask is not None:
                itc_loss = loss_fct(
                    itc_outputs.view(-1, self.model_cfg.n_classes + 1)[itc_mask],
                    itc_labels.view(-1)[itc_mask]
                )
            else:
                itc_loss = loss_fct(
                    itc_outputs.view(-1, self.model_cfg.n_classes + 1),
                    itc_labels.view(-1)
                )
